<?php
  ...
  function ToggleDelivered($receiptID){
    ...
    if(($delivered == 0)){
      $stmt = $GLOBALS['conn']->prepare("UPDATE Receipts SET delivered = 1 WHERE receiptID = :receiptID");
      $stmt->bindParam(":receiptID", $receiptID, PDO::PARAM_INT);
      $stmt->execute() or die($GLOBALS['conn']->error);

      $stmt = $GLOBALS['conn']->prepare("SELECT accountID FROM Receipts WHERE receiptID = :receiptID");
      $stmt->bindParam(":receiptID", $receiptID, PDO::PARAM_INT);
      $stmt->execute() or die($GLOBALS['conn']->error);
      $result = $stmt->fetch();
      $accountID = $result[0];
  ...
  function ToggleCancelled($receiptID){
    ...
    if(($cancelled == 0)){
      $stmt = $GLOBALS['conn']->prepare("UPDATE Receipts SET cancelled = 1 WHERE receiptID = :receiptID");
      $stmt->bindParam(":receiptID", $receiptID, PDO::PARAM_STR);
      $stmt->execute() or die($GLOBALS['conn']->error);
